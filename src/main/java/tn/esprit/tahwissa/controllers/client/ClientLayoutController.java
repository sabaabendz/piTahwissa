...
public class ClientLayoutController {
    ...
    private void loadEventView(String path) {
        try {
            // Switch to the '🎉 Gestion Événements' tab so eventContentContainer is visible
            if (clientDashboardController != null && clientDashboardController.getClientTabPane() != null) {
                TabPane tabPane = clientDashboardController.getClientTabPane();
                for (Tab tab : tabPane.getTabs()) {
                    if (tab.getText().contains("Gestion Événements")) {
            ...
            if (clientDashboardController != null && clientDashboardController.getEventContentContainer() != null) {
                AnchorPane eventContentContainer = clientDashboardController.getEventContentContainer();
                ...
                System.err.println("eventContentContainer or clientDashboardController is null");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void handleNavTransport() {
        ...
        try {
            Parent transportView = FXMLLoader.load(getClass().getResource("/fxml/transport/frontoffice_home.fxml"));
            rootPane.setCenter(transportView);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
