...
public class CreateDestinationController implements Initializable {
        ...
        new Thread(() -> {
            try {
                ...
                Platform.runLater(() -> {
                    ...
                    showAlert(Alert.AlertType.INFORMATION, "Succès", "Description générée avec succès !");
                });
            } catch (Exception e) {
                Platform.runLater(() -> {
                    btnGenerateDescription.setDisable(false);
                    btnGenerateDescription.setText("🤖 Générer Description IA");
                    showAlert(Alert.AlertType.ERROR, "Erreur", "Erreur de génération: " + e.getMessage());
                });
            }
        }).start();
    }
    ...
    private void generateAIImage() {
        ...
        btnGenerateImage.setText("🎨 Génération IA...");

        new Thread(() -> {
            try {
                byte[] imageBytes = imageService.generateDestinationImage(nom, pays);

                String timestamp = String.valueOf(System.currentTimeMillis());
                String filename = "destination_" + nom.toLowerCase().replace(" ", "_")
                        + "_" + timestamp + ".png";
                ...
                Platform.runLater(() -> {
                    txtImageUrl.setText("/images/destinations/" + filename);
